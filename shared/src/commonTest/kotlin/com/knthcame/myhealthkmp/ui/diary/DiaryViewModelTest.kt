...
class DiaryViewModelTest {
    ...
    @Test
    fun groupedDiaryEvents_returnsDiaryEvents_groupedByDate() =
        runTest {
            sut.groupedDiaryEvents.test {
                ...
                assertEquals(
                    expected =
                        mapOf(
                            now.toLocalDateTime(timeZone).date to
                                listOf(
                                    DiaryUIEvent(
                            ...
                            now.minus(1.days).toLocalDateTime(timeZone).date to
                                listOf(
                                    DiaryUIEvent(
                            ...
                            now.minus(3.days).toLocalDateTime(timeZone).date to
                                listOf(
                                    DiaryUIEvent(
    ...
    @Test
    fun delete_deletesEventFromRepository_onCall() =
        runTest {
            val event =
                DiaryUIEvent(
                    localDate = now.minus(3.days).toLocalDateTime(timeZone).date,
